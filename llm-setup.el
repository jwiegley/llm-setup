...
(defun llm-setup-get-gguf-path (model)
  ...
  (car
   (delete-dups
    (cl-loop
     for gguf in
     (cl-sort (directory-files-recursively model "\\.gguf\\'") #'string<)
     nconc (cl-loop
...
(defun llm-setup-reset ()
  ...
  (message "[llm-setup-reset] Step 1/4: Checking instances...")
...
Return the count of entries removed."
  (let ((dead-by-model (make-hash-table))
    ...
    (setq regions-to-delete
          (cl-sort regions-to-delete (lambda (a b) (> (car a) (car b)))))
